...
from .models import Address, Name, Observation, PHDCInputData, Patient, Telecom
...
class PHDCBuilder:
    ...
    def _build_base_phdc(self) -> ET.ElementTree:
        ...
        xsi_schema_location = ET.QName("http://www.w3.org/2001/XMLSchema-instance", "schemaLocation")
        ...
        pi = ET.ProcessingInstruction("xml-stylesheet", text='type="text/xsl" href="PHDC.xsl"')
    ...
    def _get_title(self) -> ET.Element:
        ...
        title.text = "Public Health Case Report - Data from the DIBBs FHIR to PHDC Converter"
        return title
    ...
    def build_body(self):
        ...
        data_type = self.input_data.type

        if data_type == "case_report":
        ...
        elif data_type == "contact_record":
            pass
        elif data_type == "lab_report":
            pass
        elif data_type == "morbidity_report":
            pass
    ...
    def _build_clinical_info(self) -> ET.Element:
        ...
        section = self._add_observations_to_section(section, data=self.input_data.clinical_info)
    ...
    def _build_social_history_info(self) -> ET.Element:
        ...
        # add observation data to section
        section = self._add_observations_to_section(section, data=self.input_data.social_history_info)
    ...
    def _build_repeating_questions(self) -> ET.Element:
        ...
        for list_of_observations in self.input_data.repeating_questions:
            filtered_observations = [obs for obs in list_of_observations if obs.obs_type == "EXPOS"]
            if not filtered_observations:
                continue

            entry = ET.Element("entry", {"typeCode": "COMP"})
            organizer = ET.Element("organizer", {"classCode": "CLUSTER", "moodCode": "EVN"})
            ...
            for observation in filtered_observations:
                if observation.components:
                    for component in observation.components:
                        ...
                        observation_element = self._build_observation(component_observation)
    ...
    def _build_observation(self, observation: Observation) -> ET.Element:
        ...
        if observation.code:
            code_element_xml = self._build_coded_element("code", **observation.code.to_attributes())
            observation_data.append(code_element_xml)

        # Add attributes to 'observation' using CodedElement for value
        if observation.value:
            value_element_xml = self._build_coded_element("value", **observation.value.to_attributes())
    ...
    def _build_patient(self, patient: Patient) -> ET.Element:
        ...
        if patient.birth_time is not None:
            e = ET.Element(
                "birthTime",
                {"value": "".join([num for num in patient.birth_time if num.isnumeric()])},
            )
