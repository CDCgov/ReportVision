...
class DIBBsMPIConnectorClient(BaseMPIConnectorClient):
    ...
    def get_block_data(self, block_criteria: Dict) -> List[list]:
        ...
        with TRACER.start_as_current_span("_get_base_query"):
        ...
        with TRACER.start_as_current_span("_organize_block_criteria"):
        ...
        with TRACER.start_as_current_span("_generate_block_query"):
        ...
        with TRACER.start_as_current_span("dal.select_results"):
    ...
    def _generate_block_query(
        ...
        for table_key, table_info in organized_block_criteria.items():
            ...
            with TRACER.start_as_current_span(f"generate_where_clause"):
            ...
            if where_clause != "" and len(where_params) > 0:
                with TRACER.start_as_current_span(f"generate_cte_query"):
                    has_column = self.dal.does_table_have_column(cte_query_table, "patient_id")
                if has_column:
                    cte_query = (
            ...
            if cte_query is not None:
                with TRACER.start_as_current_span(f"join_cte_query"):
