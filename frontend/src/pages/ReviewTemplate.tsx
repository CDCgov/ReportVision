...
import { fake_extraction_arr, fake_upload_images, fakeTemplates } from "../utils/constants.ts";
...
const ReviewTemplate: React.FC = () => {
    ...
    try {
      if (submissionArray && submissionArray.length > 0) {
        ...
        const csvData = [
          ...
          ...submissionArray.flatMap(submission => Object.entries(submission.results).map(([k, v]) => [submission.file_name, k, v.text]))  // Data rows
        ]
  ...
  useEffect(() => {
    if (pdfs.length > 0) {
        setImages(pdfs.find(pdf => pdf.file === submissionArray[submissionIndex]?.file_name)?.images || []);
    } else {
    ...
    const template = templates.filter((t) => {
      return (t?.name ?? 'empty1') === (submissionArray[index]?.template_name ?? 'empty2');
    }
    )?.[0]
    const fieldShapes = template?.pages[index]?.fieldNames.reduce((acc, curr, idx) => {
        acc[curr.label] = template.pages[index].shapes[idx].points
        return acc
    }, {}) || {};

    const maskShape = fieldShapes[selectedField || 'Patient - First Name']?.map(([x, y]) => `${x},${y}`)?.join(" ");
    const {results} = submissionArray[submissionIndex];
    const confidenceVal = 75;
    const resultsTable: ResultsTable[] = Object.entries(results).map(([k, v], idx) => {
        const overrideValue = editedValues.length === 0 ? '' :  editedValues[submissionIndex][k];
        return {
            index: idx,
            name: k,
    ...
    const overallTable: OverallTable[] = submissionArray.map((submission, idx) => {
      return {
        index: idx,
        fileName: submission.file_name,
        pageCount: pdfs.find(pdf => pdf.file === submission.file_name)?.images.length || 0,
        errors: Object.values(submission.results).reduce((sum, field) => sum + (field.confidence < 80 ? 1 : 0), 0),
        confidence: (Object.values(submission.results).reduce((sum, field) => sum + (field.confidence), 0) / Object.values(submission.results).length).toFixed(2)
      }
    })
    ...
    const redTextOnErrorFormatter = (d: string | number, _idx: number, row: Row) => {
    ...
    const editCellFormatter = (d: unknown | string, _idx: number, row: Row) => {
      return <EditableText 
        ...
        setEditedValues((prev) => {
          ...
          newValues[submissionIndex] = {...newValues[submissionIndex], [row.name]: value};
          return newValues;
        })
        ...
        textFormatter={(s) => row.isError ?  <span className="error-text"><img className="error-tex margin-right-05" src={ErrorIcon} alt="error-icon"></img>{s}</span> : s}
      />
    }
    ...
    const ReviewComponent = () => {
      if (overallTable.length === 1) {
        ...
        return (
          <ReviewBulk resultsTable={overallTable} setSubmissionIndex={setSubmissionIndex} setIsReviewing={setIsReviewing} onDownload={onDownload}  />
        )
      }
    }
  return (
