...
const ReviewBulk = ( { resultsTable, setSubmissionIndex, setIsReviewing, onDownload }: ReviewBulkProps ) => {
    ...
    const errorIconCellFormatter = (d: unknown | string, _idx: number, row: Row) => {
        return (row.errors > 0 ? <span onClick={() => onClick(row.index)} className="error-text"><img className="error-text margin-right-05" src={ErrorIcon} alt="error-icon"></img>{d}</span> : d)
      }
    
    const errorCellFormatter = (d: string, _idx: number, row: Row) => {
        return <span onClick={() => onClick(row.index)} className={row.errors > 0 ? "error-text" : ""}>{d}</span>
    }
            ...
            <div className="display-flex flex-column bg-primary-lighter width-full height-full flex-align-center">
                <div className="display-flex flex-column content-container">
                    ...
                    <p className="bulk-copy">
                        Here is your batch export. Please <strong>review all</strong> forms and correct any items with a low confidence score (CS) before downloading.
                    </p>
                    <div className="table-container">
                        <div className="width-full display-flex flex-justify review-template-header flex-align-center">
                            ...
                            <Button className="csv display-flex flex-align-center" disabled={resultsTable.reduce((sum, result) => result.errors + sum, 0) > 0} type="button" onClick={() => onDownload()}><Icon.FileDownload className="csv-icon" />Download CSV</Button>
                        </div>
                        <div className="table-div">
                            <SortableTable 
                                ...
                                defaultSort={"errors"}
                                columnNames={{ fileName: "Name", pageCount: "Page Count", errors: "Errors", confidence: "CS"}} 
                                formatters={{ errors: errorIconCellFormatter, pageCount: errorCellFormatter, confidence: errorCellFormatter, fileName: rowFormatter}}
                            />
                        </div>
