...
export const SortableTable: FC<SortableTableProps> = ({
  ...
  const sortFunc =
    SORTS[columnType] || new Intl.Collator(navigator.language).compare;

  const sortedData =
    data?.toSorted((a, b) => sortFunc(a[sortBy], b[sortBy])) || [];
  ...
  return (
    <th onClick={disabled ? () => {} : () => onClick(column)}>
      <div className="display-flex flex-row">
        ...
        {!disabled &&
          (isSortedBy ? (
            <SortOrderIcon isDescending={isDescending} />
          ) : (
            <SortIcon />
          ))}
      </div>
...
const SortOrderIcon: FC<SortIconProps> = ({ isDescending = false }) => {
  return (
    <img
      className={`margin-left-1 height-2 ${isDescending ? "" : "sort-arrow-up"}`}
      src={SortArrow}
...
const SortIcon: FC = () => {
  return (
    <img
      className={`margin-left-1 height-2 `}
      src={SortableIcon}
      alt="Sort by"
    />
  );
